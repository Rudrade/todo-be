...
public class S3Service {
    ...
    public static final String ALLOWED_EXTENSION = ".webp";

    @Value("${todo.app.user.image.bucket}")
    private String imageBucket;

    public void uploadImage(@NotNull MultipartFile image, @NotNull UUID userId) {

        if (image.getSize() > MAX_SIZE) {
            throw new InvalidDataException("Image size cannot exceed 1.5MB");
        }

        if (!image.getName().endsWith(ALLOWED_EXTENSION)) {
        ...
        try {
            ...
            s3.putObject(imageBucket, fileName, file);
